...
namespace GUI
{
    public partial class frmRelatorioCliente : Form
    {
        ...
        private DataTable populate(DataTable dt)
        {
            ...
            if (pes == 1)
            {
                da = new MySqlDataAdapter("SELECT cli_cod, cli_nome, cli_cpfcnpj, cli_rgie, cli_rsocial, cli_tipo, cli_cep, cli_endereco, cli_bairro, cli_fone, cli_cel, cli_email, cli_endnumero, cli_cidade, cli_estado FROM cliente ", cn);
            }
            else
            {
                da = new MySqlDataAdapter("SELECT cli_cod, cli_nome, cli_cpfcnpj, cli_rgie, cli_rsocial, cli_tipo, cli_cep, cli_endereco, cli_bairro, cli_fone, cli_cel, cli_email, cli_endnumero, cli_cidade, cli_estado FROM cliente where cli_nome like '%" + nome + "%'", cn);
