...
namespace GUI
{
    public partial class frmRelatorioTipoPagamento : Form
    {
        ...
        private DataTable populate(DataTable dt)
        {
            ...
            if (pes == 1)
            {
                da = new MySqlDataAdapter("SELECT tpa_cod, tpa_nome FROM tipopagamento ", cn);
            }
            else
            {
                da = new MySqlDataAdapter("SELECT tpa_cod, tpa_nome FROM tipopagamento where tpa_nome like '%" + nome + "%'", cn);
